...
placed_notes = [[[0 for k in range(9)] for j in range(9)] for i in range(9)]
...
def create_buttons():
    global DEFAULT_GRID, answer, grid, value, x_click, y_click, now_solve, auto_row, auto_column, move_left, \
        move_right, gen_click_time, note, difficulty, mode
    grid_x, grid_y = calculate_grid_position()
    ...
    if centered_coords[0][0]-30 <= (x_click - temp_x) <= centered_coords[0][-2] + 30 \
        ...
        for i in range(0, 9):
            ...
            if centered_coords[0][i] - 30 <= (x_click - temp_x):
                y = i
        highlight[1:] = [x, y]
        if value != -1 and DEFAULT_GRID[x][y] == 0:
